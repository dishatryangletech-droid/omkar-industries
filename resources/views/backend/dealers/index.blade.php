...
@section('title', 'Dealer Inquiries')
...
@section('content')
    ...
    <div class="card">
        <div class="card-datatable table-responsive pt-0">
            <table class="datatables-dealers table align-middle">
                <thead class="border-bottom">
                    <tr>
                        <th class="fw-bold">Applicant</th>
                        <th class="fw-bold">Business</th>
                ...
                <tbody>
                    @foreach($inquiries as $inquiry)
                        <tr class="cursor-pointer" data-url="#">
                            <td>
                                <span class="fw-medium d-block">{{ $inquiry->name }}</span>
                                <small class="text-muted">{{ $inquiry->email }}</small>
                            </td>
                            <td>
                                <span class="d-block">{{ $inquiry->business_name }}</span>
                                <small class="text-muted">{{ $inquiry->years_in_business }} Years</small>
                            </td>
                            <td>{{ $inquiry->city }}, {{ $inquiry->state }}</td>
                            <td>{{ $inquiry->created_at->format('d M, Y') }}</td>
                            <td>
                                <span class="badge border border-{{ $inquiry->status === 'New' ? 'danger' : ($inquiry->status === 'Read' ? 'info' : 'success') }} text-{{ $inquiry->status === 'New' ? 'danger' : ($inquiry->status === 'Read' ? 'info' : 'success') }}">
                                    {{ $inquiry->status }}
                                </span>
                            </td>
                            <td>
                                <div class="d-flex justify-content-center gap-3">
                                    <a href="#" 
                                       class="text-info waves-effect" title="View Dealer Inquiry">
                                        <i class="ti ti-eye fs-4"></i>
                                    </a>
                                    <form action="#" method="POST" id="delete-form-{{ $inquiry->id }}" title="Delete Dealer Inquiry">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn p-0 border-0 bg-transparent text-danger waves-effect delete-btn" data-id="{{ $inquiry->id }}">
...
@push('page-js')
    ...
    <script>
        $(document).ready(function() {
            var dt_dealer_table = $('.datatables-dealers');
            
            if (dt_dealer_table.length) {
                var dt_dealer = dt_dealer_table.DataTable({
            ...
            $(document).on('click', '.datatables-dealers tbody tr', function(e) {
