						...
						<ul class="pbmit-sortable-list-ul">
                           ...
                           @if($galleries && $galleries->count() > 0)
                               @foreach($galleries as $gallery)
                                   <li><a href="#" class="pbmit-sortable-link" data-sortby="{{ \Illuminate\Support\Str::slug($gallery->tab_name) }}">{{ $gallery->tab_name }}</a></li>
                               @endforeach
                           @endif
						</ul>
					</div>
                            ...
                            @foreach($galleries as $gallery)
                                @if(is_array($gallery->images) || is_object($gallery->images))
                                    @foreach($gallery->images as $image)
                                        <article class="pbmit-portfolio-style-1 col-md-6 col-lg-4 {{ \Illuminate\Support\Str::slug($gallery->tab_name) }}">
                                            <div class="pbminfotech-post-content">
                                                <div class="pbminfotech-post-warpper">
                                                    <div class="pbmit-featured-img-wrapper">
                                                        <div class="pbmit-featured-wrapper">
                                                            <img src="{{ asset('storage/' . $image) }}" class="img-fluid" alt="{{ $gallery->tab_name }}" style="height: 300px; width: 100%; object-fit: cover;">
                                                        </div>
                                                    </div>
                                                    <a class="pbmit-link" href="{{ asset('storage/' . $image) }}"></a>
                                                </div>
                                            </div>
                                        </article>
                                    @endforeach
